...
const env = {
  ...
  MONGODB_URI:
    process.env.MONGODB_URI || 'mongodb://localhost:27017/campus_exchange',
  JWT_SECRET:
    process.env.JWT_SECRET || 'dev_jwt_secret_campus_exchange_avih_2026',
  ...
  STORAGE_LOCAL_DIR:
    process.env.STORAGE_LOCAL_DIR || path.join(__dirname, '../../uploads'),
  STORAGE_MAX_FILE_SIZE_MB: parseInt(
    process.env.STORAGE_MAX_FILE_SIZE_MB || '5',
    10
  ),
  ...
  TARGET_RESPONSE_TIME_MS: parseInt(
    process.env.TARGET_RESPONSE_TIME_MS || '3000',
    10
  ),
  TARGET_AVAILABILITY_PERCENT: parseFloat(
    process.env.TARGET_AVAILABILITY_PERCENT || '99.0'
  ),
  ...
  DEFAULT_COLLEGE_NAME:
    process.env.DEFAULT_COLLEGE_NAME ||
    'Avanthi Institute of Engineering and Technology (AVIH), Gunthapalli',
  DEFAULT_COLLEGE_DOMAIN:
    process.env.DEFAULT_COLLEGE_DOMAIN || 'avih.edu.in',
};
